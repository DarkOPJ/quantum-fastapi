...
- **This skill does NOT own (see `fastapi-input-validation-vulnerability-prevention` instead):** For input/output validation, injection prevention (SQL/NoSQL/OS), SSRF, mass assignment, file upload security, and deserialization safety, see `fastapi-input-validation-vulnerability-prevention`. This skill assumes inputs are validated per that skill and focuses on auth, authorization, and security hardening only.
...
- **Primary Key Standard:** Use UUID4 (or another cryptographically random identifier) as the primary key type for all ORM models, instead of sequential auto-increment integers. Generate server-side via `uuid.uuid4()` at model instantiation (e.g. `id: Mapped[uuid.UUID] = mapped_column(default=uuid.uuid4)`). Never accept a client-supplied ID when creating a new resource. This is a structural decision made at model-definition time, not something added later — sequential IDs baked into a schema are expensive to change after the fact.
